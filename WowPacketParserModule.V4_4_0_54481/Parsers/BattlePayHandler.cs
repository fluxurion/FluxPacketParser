...
namespace WowPacketParserModule.V4_4_0_54481.Parsers
{
    ...
    public static class BattlePayHandler
    {
        ...
        private static void ReadBattlepayDisplayInfo(Packet packet, uint templateEntry, params object[] index)
        {
            ...
            var TitleSize = packet.ReadBits("TitleSize", 10);
            var Title2Size = packet.ReadBits("Title2Size", 10);
            var DescriptionSize = packet.ReadBits("DescriptionSize", 13);
            var Description2Size = packet.ReadBits("Description2Size", 13);
            var Description3Size = packet.ReadBits("Description3Size", 13);
            ...
            packet.ReadInt16("UnkFasztudja", index);
            packet.ReadByte("UnkFasztudja 2", index);

            if (HasItemModifiedAppearance)
            {
                for (int i = 0; i < 8; i++)
                {
                    packet.ReadInt16("Valamilyen Type", index, i);
                    packet.ReadInt16("ItemModifiedAppearanceID", index, i); // ???
                }

                packet.ReadInt32("asd", index);
            }
            else
            {
                var VisualsSize = 0;
                ...
                {
                    VisualsSize = packet.ReadInt32("VisualsSize", index);
                    CardType = packet.ReadInt32("CardType", index);
                    Unk4 = packet.ReadInt32("Unk4", index);
                    Unk5 = packet.ReadInt32("Unk5", index);
                }
                ...
                if (Description2Size > 1)
                    Description2 = packet.ReadWoWString("Description2", Description2Size, index);
                if (Description3Size > 1)
                    Description3 = packet.ReadWoWString("Description3", Description3Size, index);
                ...
                if (HasChoiceType)
                    ChoiceType = packet.ReadInt32("ChoiceType", index);
        ...
        public static void HandletBattlePayGetProductListResponse(Packet packet)
        {
            ...
            for (uint index = 0; index < ProductInfoSize; index++)
            {
                ...
                var ProductIDsSize = packet.ReadInt32("BundleProductIDSize", index);
                var _ChoiceType = packet.ReadInt32("ChoiceType", index);
                ...
                var fasztudja = packet.ReadInt32("fasztudja", index);
                ...
                {
                    var subproductid = packet.ReadInt32("BundleProductID", index, j);
                ...
                packet.ReadBit("unk441", index);



                if (fasztudja > 0)
                {
                    packet.ReadByte("kolagep", index);
                    packet.ReadByte("kolagep", index);
                    packet.ReadByte("kolagep", index);
                    packet.ReadByte("kolagep", index);
                    packet.ReadByte("kolagep", index);
