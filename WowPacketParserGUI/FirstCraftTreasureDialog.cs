...
public class FirstCraftTreasureDialog : Form
{
    ...
    private void ExportSqlButton_Click(object? sender, EventArgs e)
    {
        using var saveDialog = new SaveFileDialog
        {
            ...
            FileName = "first_craft_treasures.sql"
        };
        ...
        if (saveDialog.ShowDialog() == DialogResult.OK)
        {
            try
            {
                ...
                var uniqueSpells = treasures.Where(t => t.SpellID != "0").Select(t => t.SpellID).Distinct().Count();
                MessageBox.Show($"Exported {uniqueSpells} spells to {saveDialog.FileName}", "Export Complete", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
